...
public:
    ...
    void renderNextBlock(juce::AudioSampleBuffer& outputBuffer, int startSample, int numSamples) override
    {
        if (playing) // check to see if this voice should be playing
        {
            ...
            for (int sampleIndex = startSample;   sampleIndex < (startSample+numSamples);   sampleIndex++)
            {
                ...
                float gain{ env.getNextSample() };

                // your sample-by-sample DSP code here!
                float currentSample = process() * gain;

                // Check if should be turning off the note
                if (ending == true && gain <= 0)
                {
                    playing = false;
                    clearCurrentNote();
                }
    ...
    void setFrequency( int _midiNoteNumber)
    {
        float freq = juce::MidiMessage::getMidiNoteInHertz(_midiNoteNumber);
    ...
    void setParams()
    {
        params = juce::ADSR::Parameters::Parameters(0.1f, 0.1f, 1.0f, 0.1f);
...
private:
    ...
    //Create a parmeter class object
